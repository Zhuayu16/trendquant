...
$$R_t = R^{g}_t - \mathbb{1}\{\text{建仓}\}\,c_{\text{buy}} - \mathbb{1}\{\text{平仓}\}\,c_{\text{sell}}$$
...
\text{Sortino} = \frac{\bar R}{\hat\sigma_R^{-}}\sqrt{252}$$
...
4. **多重检验提示**：同时检验 6 个主动策略，Bonferroni 校正的显著性阈值
   为 $0.05/6 \approx 0.0083$；报告中同时给出未校正 p 值，供读者自行判断。
5. **改进方向**：日收益存在自相关与波动聚集时，更严格的做法是 Newey-West
   稳健 t 统计量或 block bootstrap（见 §10 与 Roadmap）。

## 9. 局限与偏差来源（诚实清单）

- **数据窥探/多重检验**：策略参数取文献与业界常用值，但仍属事后选择；
  本框架只报告单一参数化的结果，未做参数平面稳健性检验。
...
- **检验的序列相关**：t 检验假设日收益独立，策略持仓平滑会引入正自相关，
  使标准误被低估（见 §8.5 改进方向）。
